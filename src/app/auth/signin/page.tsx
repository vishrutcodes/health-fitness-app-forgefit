                        ...
                        <form onSubmit={handleSignIn} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="pl-10 bg-slate-900/50 border-forge-border text-white placeholder:text-slate-500 focus:border-forge-orange h-11" />
                                </div>
                            </div>
                            {error && (
                            ...
                            <Button type="submit" disabled={loading} className="w-full h-11 bg-linear-to-r from-forge-orange to-forge-orange-light text-white font-semibold shadow-lg shadow-forge-orange/25 hover:shadow-forge-orange/40 transition-all border-0 text-base">
                                {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><span>Sign In</span><ArrowRight className="ml-2 h-4 w-4" /></>}
                            </Button>
                        </form>
                        <div className="mt-6 text-center">
                            <p className="text-sm text-slate-400">
                                Don&apos;t have an account?{" "}
                                <Link href="/auth/signup" className="text-forge-orange hover:text-forge-orange-light font-medium transition-colors">Sign Up</Link>
                            </p>
                        </div>
                    </CardContent>
                </Card>
                <p className="text-center text-xs text-slate-600 mt-6">By signing in, you agree to start forging a better you 🔥</p>
            </motion.div>
        </div>
    );
}
...
export default function SignInPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#030712] flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-forge-orange" /></div>}>
            <SignInForm />
        </Suspense>
    );
}
